...
const Product2 = () => {
  ...
  function increment(index, vindex) {
    setProducts((p) =>
      p.map((product, i) =>
        i === index
          ? {
  ...
  function handleSelect(category, type, selectionType, checked) {
    selected((prev) => {
      if (selectionType === "radio") {
        ...
        if (checked) {
          ...
          return prev.filter(
            (item) => !(item.category === category && item.name === type.name)
          );
        }
      }
    });
        ...
        <div className="product_content">
          {products.map((product, index) => (
            <div className="main_content">
              ...
              <ul className="list">
                {product.variants.map((type, vindex) => (
                  <div>
                    <li>
                      <input
                      ...
                      />
                      {type.name} {type.quantity} {type.unit}
                      <button onClick={() => increment(index, vindex)}>
                        +
                      </button>
                      <button onClick={() => decodeURIComponent(index, vindex)}>
                        -
                      </button>
                    </li>
                  </div>
